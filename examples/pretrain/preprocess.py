...
def create_bkai_news_data(tokenizer: PreTrainedTokenizer, max_seq_length: int, short_seq_prob: float = 0.0):
    ...
    tokenized_dataset = bkai_dataset.map(sent_tokenize_function, num_proc=8, remove_columns=["text"], batched=True)
...
if __name__ == "__main__":
    ...
    wiki = create_wiki_data("datatrain.txt",tokenizer)
    news = create_bkai_news_data(tokenizer, args.max_seq_length, args.short_seq_prob)
